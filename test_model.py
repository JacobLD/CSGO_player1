    ...
    for i in range(len(change)):
        if change[i]:
            if key_is_pressed[i]:
                ...
                PressKey(KEYS[i])
                key_is_pressed[i] = not key_is_pressed[i]
                print('Pressed {}'.format(INPUT_STR[i]))
...
def main():
    ...
    while (True):
        ...
        mouse_change_bools = [inputs[0], inputs[1], inputs[2], inputs[20], inputs[21], inputs[22], inputs[23], inputs[24],
                              inputs[25], inputs[26], inputs[27], inputs[28], inputs[29], inputs[30], inputs[31]]
